...
pub async fn check_version() -> Option<AppInfos> {
    let config_text = fs::read_to_string("//192.168.3.250/soft/data2excel/version.toml");
    match config_text {
        Ok(res) => {
        ...
        Err(_) => {
            let config_text = fs::read_to_string("//192.168.100.250/soft/data2excel/version.toml");
            match config_text {
                Ok(res) => {
                    let c: AppInfos =
